      ...
      <div className="flex h-full flex-col overflow-hidden rounded-[2.5rem] border border-slate-200/80 bg-white/90 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.14)] backdrop-blur-2xl dark:border-slate-700/60 dark:bg-slate-900/90">
        {tab === "overview" ? (
          /* Warm Greeting — the "Lobby". Both the identity row and the
             status/HUD row live in ONE padded block now (no divider, no
          ...
          <div className="flex flex-shrink-0 flex-col gap-4 p-5 dark:border-slate-800 md:p-6">
              ...
              <div className="flex min-w-0 items-center gap-3">
                {currentUser?.avatar_url ? (
                  <img
                    ...
                    className={`h-14 w-14 flex-shrink-0 rounded-full object-cover shadow-lg ${verifiedRingClass(shouldShowFrame(isVerified, currentUser), "md", "glass")}`}
                  />
                ) : (
                  <div
                    className={`flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-full text-lg font-semibold text-white shadow-lg ${verifiedRingClass(shouldShowFrame(isVerified, currentUser), "md", "glass")} ${
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div
                className={`flex min-w-0 flex-1 items-center gap-3 rounded-2xl border px-4 py-2.5 shadow-sm ${
                  isVerified
