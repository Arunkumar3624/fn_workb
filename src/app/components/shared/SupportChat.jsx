    ...
    <div className="mx-auto flex h-full max-w-3xl flex-col px-4 py-6 sm:px-6 sm:py-8">
      <div className="mb-4 flex flex-shrink-0 items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl bg-[#1B3FAB]/10 text-[#1B3FAB] dark:bg-[#1B3FAB]/20 dark:text-[#5b7fe8]">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Real staff, real-time — send us anything you're stuck on.</p>
          </div>
        </div>
        {thread?.status === "RESOLVED" && (
          <span className="flex flex-shrink-0 items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-400">
        ...
        <div ref={feedRef} className="wb-scroll-clean min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-6">
          {loadError ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2 text-center">
              ...
              <p className="text-sm font-semibold text-slate-400 dark:text-slate-500">No messages yet — tell us what's going on, we're here to help.</p>
            </div>
          ) : (
              ...
              return (
                <div key={message.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                  <div className={`flex max-w-[78%] flex-col gap-1 ${isMine ? "items-end" : "items-start"}`}>
                    {isStaff && !isMine && (
                      <span className="px-1 text-[11px] font-bold uppercase tracking-wide text-[#1B3FAB] dark:text-[#5b7fe8]">WorkBridge Support</span>
                    )}
                    <div
                      className={`rounded-3xl px-4 py-3 text-sm leading-6 shadow-sm ${
                        isMine
                          ...
                          : isStaff
                            ? "rounded-bl-lg border border-[#1B3FAB]/15 bg-[#F4F6FF] text-slate-800 dark:border-[#1B3FAB]/30 dark:bg-[#1B3FAB]/10 dark:text-slate-100"
