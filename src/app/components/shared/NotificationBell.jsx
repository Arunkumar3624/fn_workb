  ...
  return (
    <>
      <button
        ...
        className="relative flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm transition-colors hover:bg-slate-50 hover:text-slate-800"
      >
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            />
            <motion.aside
              className="fixed right-0 top-0 z-[91] flex h-full w-full max-w-sm flex-col bg-white shadow-2xl"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
                    ...
                    {notifications.map((n) => (
                      <li key={n.id}>
                        <button
                          ...
                          className={`block w-full px-5 py-3.5 text-left transition-colors hover:bg-slate-50 ${
                            !n.is_read ? "bg-orange-50/50" : "bg-white"
                          }`}
                        >
