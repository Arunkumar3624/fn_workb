...
export default function LegalPageLayout({ title, lastUpdated, intro, children }) {
  ...
  return (
    <div className="bg-slate-50 px-4 py-16 sm:px-6">
      <div className="mx-auto max-w-3xl">
        <Link
          to="/"
          className="group mb-6 inline-flex items-center gap-2 text-sm font-semibold text-slate-500 transition-colors hover:text-[#FF6B35]"
        >
        ...
        {intro && <p className="mt-4 max-w-2xl text-sm leading-relaxed text-slate-600">{intro}</p>}
      </div>
      ...
      <div className="mx-auto mt-10 flex max-w-7xl flex-col gap-8 md:flex-row">
        <aside className="w-full flex-shrink-0 md:w-1/4">
          <nav className="sticky top-24 space-y-1 text-sm">
            <p className="mb-2 px-3 text-xs font-bold uppercase tracking-wider text-slate-400">On this page</p>
        ...
        <div className="min-w-0 flex-1 space-y-10 rounded-2xl border border-slate-200 bg-white p-8 shadow-sm md:p-12">
          {children}
        </div>
      </div>
    </div>
...
export function LegalSection({ id, title, children }) {
  return (
    <section id={id} className="scroll-mt-24">
      ...
      <div className="mt-3 space-y-3 text-sm leading-relaxed text-slate-600 [&_a]:font-semibold [&_a]:text-[#1B3FAB] [&_a]:underline [&_a]:underline-offset-2 [&_h3]:mt-4 [&_h3]:mb-2 [&_h3]:text-base [&_h3]:font-semibold [&_h3]:text-slate-800 [&_li]:pl-1 [&_strong]:font-semibold [&_strong]:text-slate-800 [&_ul]:list-disc [&_ul]:space-y-1.5 [&_ul]:pl-5">
        {children}
      </div>
    </section>
  );
