      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {items.map((r) => {
            ...
            return (
              <div key={r.id} className="rounded-xl border border-white/70 bg-white/60 p-5 shadow-lg shadow-slate-200/40 backdrop-blur-xl">
                <div className="mb-4 flex items-start justify-between gap-4">
                  ...
                  <div className="flex-shrink-0 text-right">
                    ...
                    <div className="mt-0.5 text-xs text-amber-600">Awaiting Verification</div>
                  </div>
                </div>

                <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="rounded-lg bg-slate-50 px-3 py-2.5 font-mono text-xs text-slate-600">
                    <span className="font-bold text-slate-800">UTR / Transaction ID:</span> {r.utr_reference}
                  </div>
                  <a
                    href={r.screenshot_url}
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-[#1B3FAB] transition-colors hover:bg-slate-50"
                  >
                ...
                {outcome ? (
                  <div className={`flex items-center gap-2 text-sm font-semibold ${outcome === "APPROVED" ? "text-emerald-600" : "text-red-600"}`}>
                  ...
                  <div className="space-y-2.5">
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-red-200"
                    />
                    <div className="flex flex-wrap gap-3">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-xs font-semibold text-red-700 transition-colors hover:bg-red-100 disabled:opacity-60"
                      >
