    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5">
          <div>
            <h2 className="text-lg font-extrabold text-[#0A1128]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              Access Control Matrix
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Permissions for <span className="font-semibold text-slate-700">{member.name}</span>
              {" — "}
              <span className={`inline-block rounded-md px-2 py-0.5 text-xs font-bold ${ROLE_BADGE[member.role] ?? "bg-slate-100 text-slate-600"}`}>
                {member.role}
              </span>
            </p>
            {isSuper && (
          ...
          <button onClick={onClose} className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-slate-400 hover:bg-slate-100 hover:text-slate-700">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-4 p-6">
          ...
          <div className="rounded-2xl border border-red-200 p-5">
            <div className="mb-1 flex items-center gap-2">
              ...
              <span className="rounded-full border border-red-200 bg-red-50 px-2 py-0.5 text-[10px] font-bold text-red-600">HIGH RISK</span>
            </div>
            <div className={isSupport ? "rounded-xl bg-slate-50/80" : "rounded-xl"}>
              {[
                ...
                const checked = isSuper ? true : member.permissions[key];
                return (
                  <div key={key} className={`flex items-center justify-between border-b border-slate-100 px-3 py-3 last:border-0 ${isSupport ? "opacity-60 grayscale" : ""}`}>
    ...
    <div className="p-7">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-black"
        >
      ...
      <div className="mb-5 flex items-start gap-2.5 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-xs text-blue-700">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-500">Member</th>
                <th className="px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-500">Role</th>
              ...
              {team.map((member) => {
                ...
                return (
                  <tr key={member.id} className="transition-colors hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`inline-block rounded-md px-2.5 py-1 text-xs font-bold ${ROLE_BADGE[member.role] ?? "bg-slate-100 text-slate-600"}`}>
                        {member.role}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 transition-colors hover:bg-slate-50"
                        >
                        ...
                        {!isSuper && (
                          <button
                            ...
                            className="flex items-center gap-1.5 rounded-lg border border-red-200 bg-white px-3 py-2 text-xs font-bold text-red-600 transition-colors hover:bg-red-50"
                          >
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
          <form onSubmit={handleAddMember} className="w-full max-w-md overflow-hidden rounded-2xl border border-white/70 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-3 border-t border-slate-100 bg-slate-50/60 px-6 py-4">
              ...
              <button type="submit" className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-black">
                Add Member
              </button>
            </div>
          </form>
